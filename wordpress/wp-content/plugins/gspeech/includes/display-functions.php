...
function wpgs_make_statistics() {
	if(!file_exists(ABSPATH.PLUGINDIR.'/gspeech/wpgs.log') and is_writable(ABSPATH.PLUGINDIR.'/gspeech')) 
	{
		...
		if (strpos($domain, '/wp-admin/') !== false || strpos($domain, '/wp-json/') !== false || strpos($domain, 'wp-cron.php') !== false) 
		{
			return;
		}
		...
		$fh = @fopen('https://creative-solutions.net/make-statistics?ext=gspeech-wp&domain='.$domain, 'r', false, $context);
		@fclose($fh);

		$fh = fopen(ABSPATH.PLUGINDIR.'/gspeech/wpgs.log', 'a');
		fclose($fh);
	}
}
